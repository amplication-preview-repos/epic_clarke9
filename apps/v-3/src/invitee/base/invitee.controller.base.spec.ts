...
const service = {
  ...
  invitees: () => FIND_MANY_RESULT,
  invitee: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Invitee", () => {
  ...
  test("GET /invitees/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/invitees"}/${nonExistingId}`)
  ...
  test("GET /invitees/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/invitees"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /invitees existing resource", async () => {
